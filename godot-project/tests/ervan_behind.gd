...
func _setup() -> void:
	...
	var s: Node = p.instantiate(); root.add_child(s); _hide(s, "Player"); _hide_labels(s); _hide(s, "NPCs/greyford_guard_letter")
	_cam = Camera3D.new(); root.add_child(_cam); _cam.make_current()
	# From behind the bar looking toward the room
	_cam.global_position = Vector3(-3.1, 1.5, -4.5)
...
func _process(_d: float) -> bool:
	...
	if img: img.save_png("res://screenshots/ervan_behind.png"); print("OK")
	quit(); return true
