...
func _setup() -> void:
	var p: PackedScene = load("res://scenes/locations/valkorn/BlackArchive.tscn")
	var s := p.instantiate(); root.add_child(s); _hide(s, "Player"); _hide_labels(s)
	_cam = Camera3D.new(); _cam.fov = 65.0; root.add_child(_cam); _cam.make_current()
	_cam.global_position = Vector3(10, 4, -12)
...
func _process(_d: float) -> bool:
	...
	get_root().get_viewport().get_texture().get_image().save_png("res://screenshots/black_archive.png")
