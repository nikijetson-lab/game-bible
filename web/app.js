// ==========================================
// ПОРТАЛ ОСТАННЬОГО МАНДРІВНИКА — ЛОГІКА ТА ГРА
// ==========================================
...
const GAME_SCENES = {
    ...
    thread_carver: {
        ...
        text: `Ви знаходите майстерню різьбяра по дереву. Старий майстер працює над викривленою гілкою і неохоче реагує на ваші запитання про Руфіна. «Багато хто приходить і йде. Чому я маю допомагати кожному Мандрівнику?»`,
        choices: [
            {
    ...
    thread_tavern: {
        ...
        text: `У брудному шинку біля причалу бармен протирає склянку. «Руфін? Так, сидів тут три дні тому. Спілкувався з однією дівчиною — куртизанкою на ім'я Касандра. Вона зараз сидить за кутовим столиком.»<br><br>Касандра дивиться на вас із підозрою і не поспішає відкривати душу першому ліпшому Мандрівнику.`,
        choices: [
            {
    ...
    ending: {
        ...
        choices: [
            {
                text: "🎮 Почати подорож заново іншим Мандрівником",
                action: () => resetGame()
            }
        ]
    }
...
function startGameFlow() {
    ...
    logDiv.innerHTML = '<div class="log-msg system">Ви розпочали подорож Останнього Мандрівника. Порожній Сезон розпочався.</div>';
    addToLog(`Мандрівник ${playerState.name} (Передісторія: ${playerState.background}) прибув до Грейфорда.`, "system");
...
function finishQuest(gateAnswer, sergeantReply) {
    ...
    endingScene.text = `
        ...
        <h2 style="font-family: var(--font-gothic); color: var(--accent-gold); margin-top: 1rem; margin-bottom: 1rem;">⚖️ ВЕРДИКТ МАНДРІВНИКА</h2>
        ${summaryText}
        ...
        <p>• <strong>Мотивація Мандрівника:</strong> ви обрали шлях <em>"${playerState.history[0].choice}"</em>.</p>
        ...
        ${repDetails.join("")}
        <br>
        <p class="gold-text" style="font-style: italic; font-weight: 600;">Шлях до поселення Тихий Шелест відкрито. Порожній Сезон чекає на Останнього Мандрівника у глибинах Хейзмуру...</p>
    `;
