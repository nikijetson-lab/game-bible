...
func _setup() -> void:
	...
	_cam.global_position = Vector3(0, 5, -6)
	_cam.look_at(Vector3(0, 2, 2), Vector3.UP)
...
func _process(_d: float) -> bool:
	_frames += 1; if _frames < 28: return false
	var img := get_root().get_viewport().get_texture().get_image()
	if img: img.save_png("res://screenshots/crafts_new.png"); print("SAVED")
	quit(); return true
